...
public class MergeRequestServiceTest {
    ...
    // 在实际中用spring方式获取单例
    private MergeRequestService mergeRequestService = new MergeRequestService();

    @Test
    public void userController(){
        mergeRequestService.init();
        // 创建 并不是马上发起请求
        for (int i = 0; i < THREAD_NUM; i++) {
            ...
            Thread thread = new Thread(() -> {
                try {
                    ...
                    Map<Long, Object> result = mergeRequestService.queryUserList(Long.valueOf(code));
